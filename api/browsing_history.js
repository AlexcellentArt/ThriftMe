...
router.get("/:user_id/:id", async (req, res, next) => {
  try {
    ...
    if (browsing_History.user_id !== user_id) {
      return next(
        gen_errors.genericAuthorityError(
          "User",
          "another Users' browsing history",
          user_id
        )
      );
    }
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const doesNotContainOnlyStrings = gen_errors.allInArrayAreType(
      "browsing history",
      looked_at_tags,
      "string",
      "looked_at_tags"
    ...
    const browsing_History = await prisma.browsing_History.create({
      data: { id, user_id, looked_at_tags },
    });
    res.json(browsing_History);
  } catch (error) {
    next(error);
